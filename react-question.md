...
date: 2020-09-25T20:37:31+08:00
---


# React Interview Questions & Answers

> 这篇文章是采自国外的一名大神，本人翻译，供大家学习
...
1. ### What is React?
    ...
    React是一个**开源的前端JavaScript库**，用于构建用户界面，尤其是单页应用。它用于处理web和移动应用的视图层。React由[Jordan Walke](https://github.com/jordwalke)创建，他是一位在Facebook工作的软件工程师。React于2011年首次部署在Facebook的News Feed上，2012年部署在Instagram上。
...
2. ### What are the major features of React?
    ...
    * 考虑到RealDOM操作成本很高，它使用**VirtualDOM**代替RealDOM。
    * 支持**服务器端渲染**。
    * 遵循**单向**的数据流或数据绑定。
    * 使用**可重用/可组合**的UI组件来开发视图。
...
3. ### What is JSX?
    ...
    *JSX*是ECMAScript的一种类似XML的语法扩展（缩写代表*JavaScript XML*）。基本上，它只是为`React.createElement()`函数提供了语法糖，使我们可以用类似HTML的模板语法来表达JavaScript。

    在下面的例子中，`<h1>`标签内的文本作为JavaScript函数返回给渲染函数。
...
4. ### What is the difference between Element and Component?
    ...
    一个*元素*是一个普通的对象，用DOM节点或其他组件来描述你想出现在屏幕上的内容。*Elements*可以在其道具中包含其他*Elements*。创建一个React元素很便宜。元素一旦被创建，就永远不会被突变。

    React Element的对象表示方法如下。
    ...
    最后用`ReactDOM.render()`渲染到DOM。

    ```html
    <div id='login-btn'>Login</div>
    ```

    而一个**组件**可以用几种不同的方式来声明。它可以是一个带有`render()`方法的类。或者，在简单的情况下，它可以被定义为一个函数。无论在哪种情况下，它都将道具作为输入，并将JSF树作为输出。
    ...
    然后JSX被转码到`React.createElement()`函数树中。
...
5. ### How to create components in React?
    ...
    1. **函数组件：**这是最简单的创建组件的方式。那些都是纯JavaScript函数，接受props对象作为第一参数，并返回React元素。
    ...
    2. **类组件：**你也可以使用ES6类来定义一个组件。上面的函数组件可以写成。
...
6. ### When to use a Class Component over a Function Component?
    ...
    如果组件需要*状态或生命周期方法*，则使用类组件，否则使用函数组件。

    *然而，从React 16.8开始，随着Hooks的加入，你可以在你的函数组件中使用状态、生命周期方法和其他只有在类组件中才有的功能。*
...
7. ### What are Pure Components?
    ...
    *`React.PureComponent`*与*`React.Component`*完全相同，只是它为你处理了`shouldComponentUpdate()`方法。当道具或状态发生变化时，*PureComponent*会对道具和状态进行浅层比较。而*Component*则不会将当前的道具和状态与下一个出场的道具和状态进行比较。因此，每当调用 "shouldComponentUpdate "时，该组件将默认重新渲染。
...
8. ### What is state in React?
    ...
    组件的*状态*是一个对象，它保存着一些信息，这些信息可能会在组件的生命周期内发生变化。我们应该总是尽量使我们的状态简单，尽量减少有状态组件的数量。
    ...
    状态类似于props，但它是私有的，完全由组件控制，即除了拥有和设置它的组件外，任何组件都不能访问它。

   **[⬆ Back to Top](#table-of-contents)**
    
9. ### What are props in React?

    *props*是组件的输入。它们是单个值或包含一组值的对象，在创建时使用类似于HTML-tag属性的命名惯例传递给组件。它们是由父组件传递给子组件的数据。

    React中道具的主要目的是提供以下组件功能。

    1. 将自定义数据传递给你的组件。
    2. 触发状态变化。
    3. 通过`this.props.reactProp`在组件的`render()`方法中使用。
    ...
    这个 "reactProp"（或者你想出来的任何东西）的名字就会成为附加在React原生道具对象上的一个属性，而这个属性本来就存在于所有使用React库创建的组件上。
...
10. ### What is the difference between state and props?
    ...
    *props*和*state*都是普通的JavaScript对象。虽然它们都持有影响渲染输出的信息，但它们与组件的功能不同。Props传递给组件的方式类似于函数参数，而state则是在组件中管理，类似于函数中声明的变量。
...
11. ### Why should we not update the state directly?
    ...
    而使用`setState()`方法。它安排对组件的状态对象进行更新。当状态发生变化时，组件会通过重新渲染做出响应。
...
12. ### What is the purpose of callback function as an argument of `setState()`?
    ...
    回调函数在setState完成和组件被渲染时被调用。由于`setState()`是**异步的**，回调函数用于任何后置操作。
...
13. ### What is the difference between HTML and React event handling?
    ...
    1. 在HTML中，事件名称要用*小写*。
       ...
       而在React中则遵循*camelCase(大驼峰)*的惯例。

        ```jsx harmony
        <button onClick={activateLasers}>
        ```

    2. 在HTML中，你可以返回 "false "来防止默认行为。

        ```html
        <a href='#' onclick='console.log("The link was clicked."); return false;' />
        ```

       而在React中，你必须显式调用`preventDefault()`。
    ...
    3. 在HTML中，你需要通过附加`()`来调用该函数。
       而在reactic中，你不应该在函数名后面加上`()`。参考第一点中的 "activateLasers "函数。
...
14. ### How to bind methods or event handlers in JSX callbacks?
    ...
    1.	**在构造函数中绑定：**在JavaScript类中，方法默认是不绑定的。同样的事情也适用于定义为类方法的React事件处理程序。通常我们在构造函数中绑定它们。
    ...
    2. **公共类字段语法：**如果你不喜欢使用绑定方式，那么可以使用*公共类字段语法*来正确绑定回调。
    ...
    **注意：**如果props作为道具传递给子组件，这些组件可能会进行额外的重新渲染。在这种情况下，考虑到性能，最好使用`.bind()`或*公共类字段语法*方法。
...
15. ### How to pass a parameter to an event handler or callback?
    ...
    你可以使用一个*箭头函数*来包围一个*事件处理程序*并传递参数。
    ...
    ```
    除了这两种方法，你还可以将参数传递给一个函数，这个函数被定义为箭头函数。
    ```jsx harmony
    <button onClick={this.handleClick(id)} />
    handleClick = (id) => () => {
...
16. ### What are synthetic events in React?
    ...
    `SyntheticEvent`是浏览器本地事件的跨浏览器封装器。它的API与浏览器的原生事件相同，包括 "stopPropagation() "和 "preventDefault()"，只是这些事件在所有浏览器中的工作原理相同。

   **[⬆ Back to Top](#table-of-contents)**
    
17. ### What are inline conditional expressions?

    你可以使用JS中提供的*if语句*或*ternary表达式*来有条件地呈现表达式。除了这些方法之外，你还可以将任何表达式嵌入到JSX中，用大括号包装，然后在后面加上JS逻辑运算符`&&`。
